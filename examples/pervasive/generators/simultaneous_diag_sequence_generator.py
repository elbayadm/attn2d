...
class SimultaneousDiagSequenceGenerator(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
    ...
    def generate(
        ...
        **kwargs
        ...
        for step in range(max_len + 1):
            ...
            step_reads = torch.zeros_like(consecutive_writes)
            for it in range(self.patience):  # Possible consecutive reads
                read_lengths = torch.min(read_lengths, dup_src_lengths)
                encoder_outs = model.forward_encoder(src_tokens, read_lengths)
                read, lprobs, avg_attn_scores = model.forward_decoder(
                    tokens[:, :step + 1], encoder_outs,
                    th=self.th
                )
                # align beams:
                if beam_size > 1 and self.align_beams:
                    # force synchronous read:
                    # print('ReaD:', read.size())
                    read = read.view(-1, beam_size)
                    collective_read = read[:, 0:1]
                    # print('Collective read', collective_read.size())
                    read = collective_read.expand(-1, beam_size).contiguous().view(-1)

                # advance in reading
                consecutive_writes[read] = 0
                # read[consecutive_writes > self.patience] = 1  # force read if too many writes
                # force read below diag
                read[read_lengths < step + 1] = 1
                read_lengths[read] += 1
                step_reads[read] += 1
                ...
                if torch.all(1 - read):
                    break
            read_lengths = torch.min(read_lengths, dup_src_lengths)
            consecutive_writes[step_reads.eq(0)] += 1
            ...
            if step < max_len:
                ...
                cand_scores, cand_indices, cand_beams = self.search.step(
                    step,
                    lprobs.view(bsz, -1, self.vocab_size).contiguous(),
            ...
            eos_mask = cand_indices.eq(self.eos) * done_reading.view(bsz, beam_size)
            finalized_sents = set()
...
class EnsembleModel(torch.nn.Module):
    ...
    def forward_encoder(self, src_tokens, read_lengths):
        """
        src_tokens :  B, Ts
        read_lengths: gives thee index of the last tokn to read. B
        """
        # maksk unread tokens:
        b, t = src_tokens.size()
        ...
        # upto the max:
        maxlen = read_lengths.max().item()
        src_tokens_clone = src_tokens_clone[:, :maxlen]
    ...
    def _decode_one(self, tokens, model, encoder_out,
        ...
        if self.incremental_states is not None:
            decoder_out = list(model.decoder.forward_read(tokens, encoder_out,
                                                          incremental_state=self.incremental_states[model],
                                                          th=th))
        else:
            decoder_out = list(model.decoder.forward_read(tokens, encoder_out, th=th))
