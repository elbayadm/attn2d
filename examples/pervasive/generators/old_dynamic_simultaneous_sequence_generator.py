...
class DynamicSimultaneousSequenceGenerator(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
    ...
    def generate(
        ...
        **kwargs
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                def get_hypo():
                    ...
                    ctx_hyp = [
                        min(c,src_lengths[idx]) for c in contexts
                    ][:len(seq)] # Remove EOS context (src length does not include eos either)
        ...
        cw = 0
        cr = 1
        src_len = src_lengths.item()
        max_read_len = src_len + 1  # Read EOS too
        for step in range(max_len + 1):
            ...
            contexts.append(min(ctx, src_len))
            ...
            # finalize hypotheses that end in eos and that have finished reading.
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, context_size, without_caching=True):
