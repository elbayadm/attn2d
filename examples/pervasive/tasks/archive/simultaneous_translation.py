...
@register_task('simultaneous_translation')
class SimultaneousTranslationTask(TranslationTask):
    """
    """
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False, **kwargs):
        ...
        # forward pass
        lprobs, controls, marginals = model(**sample['net_input'],
                                            target=target,
                                            target_lengths=target_lengths)

        loss, sample_size, logging_output = criterion(sample, lprobs, controls, marginals)
        if ignore_grad:
            loss *= 0
