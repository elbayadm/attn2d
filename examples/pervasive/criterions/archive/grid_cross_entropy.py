...
class GridCrossEntropyCriterion(FairseqCriterion):
    ...
    def __init__(self, args, task):
        ...
        self.waitk_policy = args.waitk_policy
    ...
    def forward(self, model, sample, step=-1, epoche=-1, reduce=True):
        ...
        if self.waitk_policy == 'path':
            loss, nll_loss = self.compute_loss_path(model, net_output, sample, reduce=reduce)
        elif self.waitk_policy == 'above':
            loss, nll_loss = self.compute_loss_above(model, net_output, sample, reduce=reduce)
    ...
    def compute_loss_above(self, model, net_output, sample, reduce=True):
    ...
    def compute_loss_path(self, model, net_output, sample, reduce=True):
        lprobs = model.get_normalized_probs(net_output, log_probs=True)
        B, Tt, Ts, V = lprobs.size()
        lprobs = lprobs.view(-1, lprobs.size(-1))
        target = model.get_targets(sample, net_output)
        target = target.unsqueeze(-1).expand(-1, -1, Ts)
        # flatten:
