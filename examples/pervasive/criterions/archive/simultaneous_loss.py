...
@register_criterion('simultaneous_loss')
class Simultaneous_loss(FairseqCriterion):
    def __init__(self, args, task):
        super().__init__(args, task)
        self.hmm_scale = args.hmm_scale
        self.wue_scale = args.wue_scale
        self.transition_scale = args.transition_scale
        self.alpha = args.control_scale
        self.delay_scale = args.delay_scale
        self.eps = 1e-6
        self.delay_margin = args.delay_margin

    def add_args(parser):
        """Add criterion-specific arguments to the parser."""
        parser.add_argument('--hmm-scale', default=1., type=float, metavar='D')
        parser.add_argument('--control-scale', default=1., type=float, metavar='D')
        parser.add_argument('--transition-scale', default=1., type=float, metavar='D')
        parser.add_argument('--delay-scale', default=0., type=float, metavar='D')
        parser.add_argument('--wue-scale', default=0., type=float, metavar='D')
        parser.add_argument('--delay-margin', default=0., type=float, metavar='D')

    def forward(self, sample, lprobs, controls, wst, rst, reduce=True):
        target = sample['target']
        device = target.device
        N, Tt, Ts = lprobs.size()
        # writer
        writing_loss = - torch.sum(wst * lprobs).to(device)
        controlling_loss = - torch.sum(rst * controls).to(device)
