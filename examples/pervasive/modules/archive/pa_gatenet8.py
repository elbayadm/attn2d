...
class PAGateNet8(nn.Module):
    ...
    def __init__(self, num_init_features, args):
        ...
        for _ in range(num_layers):
            ...
            self.gates.append(_GateLayer(num_features))
            self.gates.append(_GateLayer(num_features))
    ...
    def forward_stats(self, x, 
        ...
        cumul_stats, fd_stats = [], []
        features = self.gate_embeddings(x)
        cumul_stats.append(get_stats(features))
        for layer, gate in zip(self.blocks, self.gates):
            ...
            ft = gate(xlayer)
            cumul_stats.append(get_stats(ft))
            features += ft
            x = self.depth_gate(x + xlayer)
            fd_stats.append(get_stats(x))
        return (cumul_stats, fd_stats)
...
class _GateLayer(nn.Module):
    def __init__(self, num_features):
        super().__init__()
        self.scale = nn.Parameter(torch.Tensor(1,1,1,num_features).fill_(0.5**0.5))
