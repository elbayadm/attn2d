...
Embeddings gate + attn/ffn gates per layer
# Two depth-gates

Pick Hconv from tilde H^n
"""

class PAGateNet10(nn.Module):
    ...
    def __init__(self, num_init_features, args):
        ...
        self.gate_embeddings = _GateLayer(num_features, 1/(2*num_layers+1))
        self.depth_gate_1 = _GateLayer(num_features)
        self.depth_gate_2 = _GateLayer(num_features)

        for _ in range(num_layers):
            self.blocks.append(_ConvLayer(num_features, args))
            self.blocks.append(_FFLayer(num_features, args))
            self.gates.append(_GateLayer(num_features, 1/(2*num_layers+1)))
            self.gates.append(_GateLayer(num_features, 1/(2*num_layers+1)))
...
class _GateLayer(nn.Module):
    def __init__(self, num_features, init=0.5**0.5):
        super().__init__()
        self.scale = nn.Parameter(torch.Tensor(1,1,1,num_features).fill_(init))
