...
class _ResLayer(nn.Module):
    ...
    def __init__(self, num_features, kernel_size, args):
        ...
        self.ln11 = nn.LayerNorm(num_features)
        self.ln12 = nn.LayerNorm(num_features)

        self.fc1 = Linear(num_features, ffn_dim)
        self.fc2 = Linear(ffn_dim, num_features)
        self.ln21 = nn.LayerNorm(num_features)
        self.ln22 = nn.LayerNorm(num_features)
    ...
    def forward(self, x, incremental_state):
        ...
        x = x.permute(0, 2, 3, 1)
        x = self.ln12(residual + self.ln11(residual + x))  # Renorm
        # FFN:
        residual = x
        x = self.fc1(x)
        x = F.relu(x)
        ...
        if self.drop_rate:
            x = F.dropout(x, p=self.drop_rate, training=self.training)
        x = self.ln22(residual + self.ln21(residual + x))  # Renorm
        return x
