...
@register_model('double_attn2d_dynamic_ll')
class Attn2dWaitkModel(FairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--num-controller-layers', type=int, help='number of layers')
...
class Attn2dEncoder(FairseqEncoder):
    def __init__(self, args,  dictionary, embed_tokens, ctrl_embed_tokens, left_pad=False):
        ...
        self.ctrl_embed_tokens = ctrl_embed_tokens
        ...
        self.ctrl_embed_positions = PositionalEmbedding(
            self.max_source_positions, 
            embed_dim, self.padding_idx,
            left_pad=args.left_pad_source,
            learned=args.learned_pos,
    ...
    def forward(self, src_tokens, src_lengths=None, **kwargs):
        xctrl = self.embed_scale * self.ctrl_embed_tokens(src_tokens)
        if self.ctrl_embed_positions is not None:
            xctrl += self.ctrl_embed_positions(src_tokens)
        xctrl = self.embedding_dropout(xctrl)
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        if encoder_out['ctrl_encoder_out'] is not None:
            encoder_out['ctrl_encoder_out'] = \
                encoder_out['ctrl_encoder_out'].index_select(0, new_order)
...
class Attn2dDecoder(FairseqIncrementalDecoder):
    ...
    def forward_train(self, prev_output_tokens, encoder_out, target, **kwargs):
        ...
        ctrl_src_emb = encoder_out['ctrl_encoder_out']  # B, Ts, ds 
        ...
        ctrl_positions = self.ctrl_embed_positions(
            prev_output_tokens,
            incremental_state=None,
        ) if self.ctrl_embed_positions is not None else None
        ...
        ctrl_tgt_emb = self.embed_scale * self.ctrl_embed_tokens(prev_output_tokens)
        if ctrl_positions is not None:
            ctrl_tgt_emb += ctrl_positions
        ctrl_tgt_emb = self.embedding_dropout(ctrl_tgt_emb)
        ...
        # Control grid:
        ctrl_src_emb = _expand(ctrl_src_emb, 1, tgt_length)  # B, Tt, Ts, ds
        ctrl_tgt_emb = _expand(ctrl_tgt_emb, 2, src_length)  # B, Tt, Ts, dt
        xctrl = torch.cat((ctrl_src_emb, ctrl_tgt_emb), dim=3)   # B, Tt, Ts, C=ds+dt

        observations = self.controller_feat(xctrl)
        # pass through dense convolutional layers
        encoder_mask = encoder_out['encoder_padding_mask']
        x = self.net(
            x, 
...
@register_model_architecture('double_attn2d_dynamic_ll', 'double_attn2d_dynamic_ll')
