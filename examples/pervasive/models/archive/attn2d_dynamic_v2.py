...
from fairseq.modules import (
    ...
    HMMControls3, LLControls,
    GridMAX,
)


@register_model('attn2d_dynamic_v2')
class Attn2dWaitkModel(FairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--controller-input', type=str, default='row')
        parser.add_argument('--controller-type', type=str, default='hmm') 
...
class Attn2dDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args,  dictionary, embed_tokens, left_pad=False):
        ...
        if args.controller_type == 'hmm':
            self.controller = HMMControls3(
                args,
                controller_dim
            )
        elif args.controller_type == 'll':
            self.controller = LLControls(
                    args,
                    controller_dim
                )
        else:
            raise ValueError('Unknown type of controller', args.controller_type)
    ...
    def decide(self, prev_output_tokens, encoder_out, context_size):
        # source embeddings
        src_emb = encoder_out['encoder_out'].clone()[:, -1]  # B, ds 
        prev_output_tokens = prev_output_tokens[:, -1:]  # B,1,dt
        # target embeddings:
        positions = self.embed_positions(
            prev_output_tokens,
        ) if self.embed_positions is not None else None
        ...
        obs = torch.cat((src_emb, tgt_emb.squeeze(1)), dim=-1)   # B, C=ds+dt
        controls = self.controller.predict_read_write(obs)  # B, 2
        pwrite = torch.exp(controls[:,-1])
        return pwrite 
...
@register_model_architecture('attn2d_dynamic_v2', 'attn2d_dynamic_v2')
def base_architecture(args):
    ...
    # HMM
    args.num_controller_layers = getattr(args, 'num_controller_layers', 2)
    args.detach_controls = getattr(args, 'detach_controls', False)
    args.controller_input = getattr(args, 'controller_input', 'embed,feat')
    args.controller_type = getattr(args, 'controller_type', 'hmm')
