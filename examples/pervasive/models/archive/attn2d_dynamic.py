...
from fairseq.modules import (
    ...
    HMMControls, HMMControls2,
    GridMAX,
)


@register_model('attn2d_dynamic')
class Attn2dWaitkModel(FairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--hmm-type', type=int, help='HMM') 
...
class Attn2dDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args,  dictionary, embed_tokens, left_pad=False):
        ...
        # Controller:
        self.controller_input = args.controller_input.split(',')
        print('Controller input:', self.controller_input)
        controller_dim = 0
        ...
        if 'feat' in self.controller_input:
            controller_dim += self.output_dim

        if args.hmm_type == 1:
            self.hmm = HMMControls(
                args,
                controller_dim
            )
        elif args.hmm_type == 2:
            self.hmm = HMMControls2(
                args,
                controller_dim
            )
    ...
    def decide(self, prev_output_tokens, encoder_out, context_size):
        ...
        controls = self.hmm.predict_read_write(obs)  # B, 2
...
@register_model_architecture('attn2d_dynamic', 'attn2d_dynamic')
def base_architecture(args):
    ...
    args.hmm_type = getattr(args, 'hmm_type', 1)
