...
class Convs2s2D(nn.Module):
    ...
    def sample(self, data_src, scorer, kwargs={}):
        ...
        if beam_size > 1:
            # return self.sample_beam_new(data_src, scorer, kwargs)
    ...
    def sample_obs(self, data_src, kwargs={}):
        if isinstance(self.src_embedding, ConvEmbedding):
            return self.sample_conv_emb(data_src, kwargs)
        ...
        for t in range(max_length):
            ...
            logits = logits[:, 1:]  # remvoe pad
            npargmax = 1 + logits.data.cpu().numpy().argmax(axis=-1)
            # Aletrnative if padding_idx != 0
            # logits[:, self.padding_idx] = -math.inf
            # npargmax = logits.data.cpu().numpy().argmax(axis=-1)
            next_preds = torch.from_numpy(npargmax)
            next_preds = next_preds.view(-1, 1).cuda()
            seq.append(next_preds)
            next_emb = self.trg_embedding.single_token(next_preds, t)
            next_emb = _expand(next_emb.unsqueeze(2), 2, Ts)
            trg_emb = torch.cat((trg_emb,
                                 next_emb),
                                dim=1)
            src_emb = _expand(src_emb_, 1, t+2)
            if t >= 1:
                # stop when all finished
                unfinished = torch.add(
                        torch.mul((next_preds ==
        ...
        seq = torch.cat(seq, 1)
        print('Timing regular:', timing)
        return seq, None  # FIXME return scores as well

    def sample_conv_emb(self, data_src, kwargs={}):
        batch_size = data_src['labels'].size(0)
        ...
        max_length = int(kwargs.get('max_length_a', 0) * Ts +
                         kwargs.get('max_length_b', 50))
        src_emb = self.src_embedding(data_src).unsqueeze(1)
        src_emb_ = src_emb
        seq = []
        for t in range(max_length):
            print('trg labels:', trg_labels)
            trg_emb = self.trg_embedding({"labels": trg_labels,
                                          "lengths": None})
            trg_emb = _expand(trg_emb.unsqueeze(2), 2, Ts)
            X = torch.cat((src_emb, trg_emb), dim=3)
            Y = self._forward(X, data_src["lengths"])
    ...
    def sample_beam_new(self, data_src, scorer=None, kwargs={}):
        """
        ...
        src_emb = self.src_embedding(data_src)
        Ts = src_emb.size(1)  # source sequence length
        src_emb = src_emb.repeat(beam_size, 1, 1).unsqueeze(1)  # N*B, 1, Ts, d
        src_emb_ = src_emb
        # source lengths:
        ...
        for t in range(max_length):
            ...
            src_emb = _expand(src_emb_, 1, trg_emb.size(1))
            X = torch.cat((src_emb, trg_emb), dim=3)
            Y, attn = self.update(X, src_lengths, track=True)
            proj = self.prediction_dropout(Y[:, -1, :])
            logits = F.log_softmax(self.prediction(proj), dim=1)
            word_lk = logits.view(
                beam_size,
            ...
            for b in range(batch_size):
                if beam[b].done():
                    continue
                idx = batch_idx[b]
                if not beam[b].advance(word_lk.data[idx], attn):
                    active += [b]
                trg_emb_prev = trg_emb.view(beam_size,
                                            remaining_sents,
                                            trg_emb.size(1),
                                            Ts,
                                            self.trg_embedding.dimension)
                trg_emb = trg_emb_prev[
                    beam[b].get_current_origin()
                ].view(-1, trg_emb.size(1), Ts, self.trg_embedding.dimension)
            if not active:
                break
            ...
            def update_active(t):
                ...
                # updating src_emb & trg_emb (N, 1, Ts, d) or (N, Tt, 1, d)
                initial = t.size()
            ...
            src_emb_ = update_active(src_emb_)
            trg_emb = update_active(trg_emb)
        ...
        for b in range(batch_size):
            ...
            try:
                ...
                raise ValueError('ks empty: %s' % str(ks))
            allHyp += [hyps]
        self.net.reset_buffers()
        self.trg_embedding.reset_buffers()
