...
closs=1
wloss=1
special=" --observe-encoder --control-gate-layers 1 "
EXP="dynamic_encdec_layers1_waitk_w${wloss}_c${closs}_penal${penal}_k${kmin}_k${kmax}_2506"
...
CUDA_VISIBLE_DEVICES=$CUDA python train.py $DATA --distributed-port 28275 --distributed-world-size 1 --user-dir $FAIR/examples/simultaneous \
    -a ${arch} --optimizer adam -s de -t en $special \
