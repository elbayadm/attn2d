...
closs=1
wloss=1
EXP="wmt_encdec_dynamic_waitk_w${wloss}_c${closs}_penal${penal}_k${kmin}_k${kmax}_2506"
...
arch=dynamic_simultaneous_transformer_big
...
CUDA_VISIBLE_DEVICES=$CUDA python train.py $DATA --distributed-port 28275 --distributed-world-size 1 --user-dir $FAIR/examples/simultaneous \
    -a ${arch} --optimizer adam -s de -t en --fp16 --observe-encoder \
    ...
    --share-all-embeddings --control-gate-dropout 0.2 --control-oracle-penalty $penal \
    --pretrained /scratch/zeus/melbayad/work/checkpoints/transformer_waitk_wmt15_deen_sample_waitk/sweep_waitk1to300_tok2000_freq114_g2/seed1/checkpoint_best.pt
