...
clayers=2
glayers=2
special=" --copy-embeddings --copy-network "
#--control-remove-writer-dropout
#--restrain-writer
#--observe-encoder

EXP="envi_branch_dynamic_wl${wloss}_cl${closs}_k${kmin}_${kmax}_pnl${penal}_clyr${clayers}_glyr${glayers}_copyemb+net"
...
CUDA_VISIBLE_DEVICES=$CUDA python train.py $DATA --distributed-port 28275 --distributed-world-size 1 --user-dir $FAIR/examples/simultaneous \
    ...
    --share-decoder-input-output-embed  --control-gate-dropout 0. --control-num-encoder-layers ${clayers} --control-num-decoder-layers ${clayers} --control-gate-layers $glayers --control-oracle-penalty $penal \
