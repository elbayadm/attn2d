...
wloss=1
layers=2
EXP="frozen_nodp_dynamic_envi_gelu_layers${layers}_w${wloss}_c${closs}_penal${penal}_k${kmin}_k${kmax}_2606"
special=" --control-gate-layers ${layers} --control-nonlin gelu --freeze-writer --control-remove-writer-dropout "
#
#--observe-encoder 
#--control-detach
#--restrain-writer 
...
CUDA_VISIBLE_DEVICES=$CUDA python train.py $DATA --distributed-port 28275 --distributed-world-size 1 --user-dir $FAIR/examples/simultaneous \
    ...
    --share-decoder-input-output-embed  --control-gate-dropout 0.1 --control-oracle-penalty $penal \
