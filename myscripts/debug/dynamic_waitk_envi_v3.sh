...
wloss=1
layers=2
EXP="frozen_dynamic_envi_gelu_layers${layers}_detach_encdec_restrain_w${wloss}_c${closs}_penal${penal}_k${kmin}_k${kmax}_2506"
special=" --control-gate-layers ${layers} --control-detach --observe-encoder --control-nonlin gelu --restrain-writer --freeze-writer"
...
CUDA_VISIBLE_DEVICES=$CUDA python train.py $DATA --distributed-port 28275 --distributed-world-size 1 --user-dir $FAIR/examples/simultaneous \
    ...
    --share-decoder-input-output-embed  --control-gate-dropout 0. --control-oracle-penalty $penal \
